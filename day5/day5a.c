...
long int search(RANGE arr[], int size, long int value);
...
int main(void) {
  ...
  long int lowest_location = LONG_MAX;
  for (int i = 0; i < seed_index; i++) {
    long int final_loc = process_seed(seeds[i], map_range, map_count);
    if (final_loc < lowest_location) {
      lowest_location = final_loc;
    }
  }
...
long int search(RANGE arr[], int size, long int value) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (value >= start && value <= end) {
      val = (arr[i].dist_start - start) + value;
      return val;
    }
  }
  return value;
...
long int process_seed(long int value, RANGE map_range[MAX_MAPS][MAX_RANGES],
  ...
  for (int j = 0; j < MAX_MAPS; j++) {
    seed = search(map_range[j], map_count[j], seed);
  }
  return seed;
}
